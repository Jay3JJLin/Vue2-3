<template>
  <div class="box">
    ...
    <!-- 测试访问模块中的actions -- 原生 -->
    <br />
    <button @click="updateUserAction">一秒后更新个人信息</button>
    <button @click="updateThemeAction">一秒后更新主题色</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updateUserAction () {
      // 调用action - dispatch
      // 其实就相当于 dispatch调action, action异步执行完调mutation
    ...
    updateThemeAction () {
      this.$store.dispatch('setting/setThemeAction', 'green')
    }
  }
}
</script>
