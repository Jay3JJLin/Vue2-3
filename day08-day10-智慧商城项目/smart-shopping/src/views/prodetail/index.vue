...
export default {
  ...
  data () {
    return {
      ...
      commentsList: [], // 评价列表
      defaultImg,
      showPannel: false, // 控制弹层的显示隐藏,默认隐藏
      mode: 'cart', // 标记弹层状态
      buyCount: 1, // 数字框绑定的数据
      cartTotal: 0 // 购物车商品数量(角标)
    }
  },
  computed: {
    goodsId () {
  ...
  methods: {
    ...
    addFn () {
      this.mode = 'cart' // 加入购物车
      this.showPannel = true
    },
    buyFn () {
      this.mode = 'buyNow' // 立即购买
      this.showPannel = true
    },
    async addCart () {
      // 没有登录，弹了确认框
      ...
      // 存在token
      const { data } = await addCart(this.goodsId, this.buyCount, this.detail.skuList[0].goods_sku_id)
